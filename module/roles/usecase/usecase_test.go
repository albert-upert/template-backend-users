...
func TestUseCase_Roles(t *testing.T) {
	...
	t.Run("FindAll", func(t *testing.T) {
		...
		filter := domain.RoleFilter{UniversityId: "inst-1"}
	...
	t.Run("Create", func(t *testing.T) {
		...
		mockRepo.On("FindByName", mock.Anything, role.UniversityId, role.Name).Return((*domain.Role)(nil), pgx.ErrNoRows).Once()

		mockRepo.On("Store", mock.Anything, mock.MatchedBy(func(r *domain.Role) bool {
			return r.Name == role.Name && r.UniversityId == role.UniversityId
		})).Return(nil).Once()
	...
	t.Run("Update", func(t *testing.T) {
		ctx := context.Background()
		role := &domain.Role{Id: "r1", UniversityId: "inst-1", Name: "Role", Permissions: []string{"p2"}}
		current := &domain.Role{Id: "r1", UniversityId: "inst-1", Name: "Role", Permissions: []string{"p1"}}
	...
	t.Run("Create_Duplicate", func(t *testing.T) {
		ctx := context.Background()
		role := &domain.Role{UniversityId: "1", Name: "R"}
	...
	t.Run("Update_ValidationFail", func(t *testing.T) {
		ctx := context.Background()
		role := &domain.Role{Id: "r1", UniversityId: "inst-1", Name: "Changed"}
		current := &domain.Role{Id: "r1", UniversityId: "inst-1", Name: "Original"}
	...
	t.Run("Create_StoreFail", func(t *testing.T) {
		ctx := context.Background()
		role := &domain.Role{UniversityId: "i", Name: "N"}
