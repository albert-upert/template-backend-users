...
func TestUserHandler_GetUsers(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		usersList := []*domain.User{
			{Id: "u1", UniversityId: "inst-1", ExternalSubject: "sub1", Status: "active"},
		}
		count := int64(1)

		mockUseCase.On("FindAll", mock.Anything, mock.MatchedBy(func(f domain.UserFilter) bool {
			return f.UniversityId == "inst-1" && f.Pagination.Page == 1 && f.Pagination.Size == 10
	...
	t.Run("WithFilters", func(t *testing.T) {
		mockUseCase.On("FindAll", mock.Anything, mock.MatchedBy(func(f domain.UserFilter) bool {
			return f.UniversityId == "inst-1" && f.Search == "test" && f.Status == "active"
...
func TestUserHandler_AssignRole(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		body := map[string]interface{}{
			"role_id":       "r1",
			"group_id":      "g1",
			"university_id": "inst-1",
		}
		reqBytes, _ := json.Marshal(body)

		mockUseCase.On("AssignRole", mock.Anything, mock.MatchedBy(func(cmd domain.AssignRoleCommand) bool {
			return cmd.UserId == "u1" && cmd.RoleId == "r1" && cmd.InstitutionId == "i1" && cmd.UniversityId == "univ-1"
		})).Return("ur1", nil).Once()
	...
	t.Run("AppError", func(t *testing.T) {
		body := map[string]interface{}{"role_id": "r1", "group_id": "g1", "university_id": "inst-1"}
		reqBytes, _ := json.Marshal(body)
